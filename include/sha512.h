...
typedef struct { uint64_t h[8]; uint64_t total_len; uint8_t buf[PURE_SHA512_BLOCK_LEN]; size_t buflen; } pure_sha512_ctx;
...
void pure_sha512(const uint8_t *data, size_t len, uint8_t out[PURE_SHA512_DIGEST_LEN]);
void pure_sha512_hex(const uint8_t digest[PURE_SHA512_DIGEST_LEN], char out[129]);
